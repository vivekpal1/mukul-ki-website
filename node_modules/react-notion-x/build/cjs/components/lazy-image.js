...
var LazyImage = function (_a) {
    ...
    var _e = (0, context_1.useNotionContext)(), recordMap = _e.recordMap, zoom = _e.zoom, previewImages = _e.previewImages, forceCustomImages = _e.forceCustomImages, components = _e.components;
    ...
    var onLoad = react_1["default"].useCallback(function (e) {
    ...
    var attachZoom = react_1["default"].useCallback(function (image) {
        if (zoomRef.current && image) {
            ;
            zoomRef.current.attach(image);
        }
    }, [zoomRef]);
    var attachZoomRef = react_1["default"].useMemo(function () { return (zoomable ? attachZoom : undefined); }, [zoomable, attachZoom]);
    if (previewImage) {
        ...
        if (components.image) {
            ...
            return (react_1["default"].createElement(components.image, { src: src, alt: alt, style: style, className: className, width: previewImage.originalWidth, height: previewImage.originalHeight, blurDataURL: previewImage.dataURIBase64, placeholder: 'blur', priority: priority, onLoad: onLoad }));
        }
        return (react_1["default"].createElement(react_lazy_images_1.LazyImageFull, __assign({ src: src }, rest, { experimentalDecode: true }), function (_a) {
            ...
            return (react_1["default"].createElement("div", { className: (0, utils_1.cs)('lazy-image-wrapper', isLoaded && 'lazy-image-loaded', className), style: wrapperStyle },
                react_1["default"].createElement("img", { className: 'lazy-image-preview', src: previewImage.dataURIBase64, alt: alt, ref: ref, style: style, decoding: 'async' }),
                react_1["default"].createElement("img", { className: 'lazy-image-real', src: src, alt: alt, ref: attachZoomRef, style: __assign(__assign({}, style), imgStyle), width: previewImage.originalWidth, height: previewImage.originalHeight, decoding: 'async', loading: 'lazy' })));
        }));
    }
    else {
        ...
        */
        if (components.image && forceCustomImages) {
            return (react_1["default"].createElement(components.image, { src: src, alt: alt, className: className, style: style, width: null, height: height || null, priority: priority, onLoad: onLoad }));
        }
        // Default image element
        return (react_1["default"].createElement("img", __assign({ className: className, style: style, src: src, alt: alt, ref: attachZoomRef, loading: 'lazy', decoding: 'async' }, rest)));
    }
};
